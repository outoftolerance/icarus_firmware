...
/*------------------------------Constructor Methods------------------------------*/
SimpleLog::SimpleLog(Stream& port, LOG_LEVELS log_level) :
  output_(port),
...
SimpleLog::SimpleLog(Stream& port, RTC_DS3231* rtc, LOG_LEVELS log_level) :
  output_(port),
  clock_(rtc),
  log_level_(log_level)
{
...
void SimpleLog::event(LOG_LEVELS level, const char message[])
{
    if(level >= log_level_)
    {
        String preamble;
...
void SimpleLog::event(LOG_LEVELS level, const char message[], const float data)
{
    if(level >= log_level_)
    {
        String preamble;
...
void SimpleLog::event(LOG_LEVELS level, const char message[], const double data)
{
    if(level >= log_level_)
    {
        String preamble;
...
void SimpleLog::event(LOG_LEVELS level, const char message[], const int data)
{
    if(level >= log_level_)
    {
        String preamble;
...
void SimpleLog::getPreamble_(LOG_LEVELS level, String& preamble)
{
